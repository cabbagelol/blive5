...
export class StaffgaugeComponent implements OnInit {
  ...
  onReadyStaffgauge () {
    ...
    document.onmousemove = e => {
      ...
      if (this_.Ruler.Mouse) {
        const Mouse = {x: e.clientX - 25, y: e.clientY - 25};
        Mouse.x += this_.Ruler.direction ? 25 : 0;
        this_.Ruler.Mouse = Mouse;// 赋值鼠标位
      }
    };
  }
  ...
  _Ruler_Guides_dolly(key) { // 参考线 ~ 移动
    ...
    if (key === false) {
      ...
      if (is_Guides !== false) { // 参考线添加/移动共享方法
        ...
        if (this_.Ruler.direction) {
          if (Guides.standard && Guides.distance <= 20) {
            this_.Ruler.Guides.value.splice(is_Guides, 1);
          } else if (Guides.distance >= $('#App')[0].offsetWidthh - 25) {
            this_.Ruler.Guides.value.splice(is_Guides, 1);
          }
        } else if (Guides.distance <= 20) { // 删除该元素
          this_.Ruler.Guides.value.splice(is_Guides, 1);
        }
        this_.Ruler.Guides.index = false; // 取消定位
      }
    } else {
      if (this_.$_Ruler_Guides_ >= 0) { // 有bug，如果存在修正
        clearInterval(this_.$_Ruler_Guides_);
      }
